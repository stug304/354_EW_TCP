Gamicus.Platformer = function(G) {

 G.TileLayer = G.Sprite.extend({
    ...
    load: function(dataAsset) {
      var data = _.isString(dataAsset) ?  G.asset(dataAsset) : dataAsset;
      this.p.tiles = data;
      ...
      if(p.tiles[tileY] && p.tiles[tileY][tileX] > 0) {
        ...
        switch(which) {
          case 'top':
            ...
            this.colBounds.destY = (tileY+1)*p.tileH + p.y + G.dx;
            break;
          case 'bottom':
            this.colBounds.destX = x;
            this.colBounds.destY = tileY*p.tileH + p.y - G.dx;
            break;
          case 'left':
            this.colBounds.destX = (tileX+1)*p.tileW + p.x + G.dx;
            this.colBounds.destY = y;
            break;
          case 'right':
            this.colBounds.destX = tileX*p.tileW + p.x - G.dx;
            this.colBounds.destY = y;
            break;
        }
    ...
    draw: function(ctx) {
      var p = this.p,
          ...
          viewW = G.width / viewport.scale,
          viewH = G.height / viewport.scale,
  ...
  G.register('2d',{
    ...
    step: function(dt) {
      ...
      while(dtStep > 0) {
        ...
        p.vx += p.ax * dt + G.gravityX * dt * p.gravity;
        p.vy += p.ay * dt + G.gravityY * dt * p.gravity;
        p.x += p.vx * dt;
        p.y += p.vy * dt;
  ...
  G.PlatformStage = G.Stage.extend({
